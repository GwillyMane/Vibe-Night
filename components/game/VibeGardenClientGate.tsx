...
import type { VibeGardenGameProps } from "./vibe-garden/VibeGardenGame";

const CHUNK_LOAD_ATTEMPTS = 4;
const CHUNK_RETRY_BASE_MS = 400;
const PRODUCT_TITLE_FALLBACK = "Vibe Garden";

async function importVibeGardenGame(): Promise<{ default: ComponentType<VibeGardenGameProps> }> {
  for (let attempt = 0; attempt < CHUNK_LOAD_ATTEMPTS; attempt++) {
    try {
      return await import(
        /* webpackChunkName: "vibe-garden-game", webpackPrefetch: true */ "./vibe-garden/VibeGardenGame"
      );
    } catch (e) {
  ...
  throw new Error("importVibeGardenGame: unreachable");
}
...
function GameLoading({ slow }: { slow: boolean }) {
  return (
    <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-3 px-4 text-center">
      ...
      <p className="max-w-sm font-body text-sm text-white/50">Preparing garden physics and vibe art.</p>
      {slow ? (
...
class GardenErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("[Vibe Garden] render error", error, info.componentStack);
  }
    ...
    if (this.state.error) {
      return (
        <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-4 px-4 text-center">
          <p className="font-display text-xl font-black uppercase text-gvc-gold">Something wilted</p>
...
export interface VibeGardenClientGateProps {
  onExitToLibrary?: () => void;
}

export default function VibeGardenClientGate({ onExitToLibrary }: VibeGardenClientGateProps) {
  const [Game, setGame] = useState<ComponentType<VibeGardenGameProps> | null>(null);
  ...
  useEffect(() => {
    ...
    const boot = window.setTimeout(() => {
      ...
      failTimer = window.setTimeout(() => {
        if (!cancelled && !loadedRef.current) setLoadErr("Timed out loading Vibe Garden (45s).");
      }, 45000);

      importVibeGardenGame()
        .then((m) => {
          if (cancelled) return;
          if (!m?.default) throw new Error("Vibe Garden module missing default export.");
