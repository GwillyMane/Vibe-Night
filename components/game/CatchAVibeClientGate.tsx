...
import type { CatchAVibeGameProps } from "./catch-a-vibe/CatchAVibeGame";

const CHUNK_LOAD_ATTEMPTS = 4;
const CHUNK_RETRY_BASE_MS = 400;
const PRODUCT_TITLE_FALLBACK = "Catch A Vibe";

async function importCatchAVibeGame(): Promise<{ default: ComponentType<CatchAVibeGameProps> }> {
  for (let attempt = 0; attempt < CHUNK_LOAD_ATTEMPTS; attempt++) {
    try {
      return await import(
        /* webpackChunkName: "catch-a-vibe-game", webpackPrefetch: true */ "./catch-a-vibe/CatchAVibeGame"
      );
    } catch (e) {
  ...
  throw new Error("importCatchAVibeGame: unreachable");
}
...
function GameLoading({ slow }: { slow: boolean }) {
  return (
    <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-3 px-4 text-center">
      ...
      <p className="max-w-sm font-body text-sm text-white/50">Preparing catch arena and vibe art.</p>
      {slow ? (
...
class CatchErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("[Catch A Vibe] render error", error, info.componentStack);
  }
    ...
    if (this.state.error) {
      return (
        <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-4 px-4 text-center">
          <p className="font-display text-xl font-black uppercase text-gvc-gold">Something slipped</p>
...
export interface CatchAVibeClientGateProps {
  onExitToLibrary?: () => void;
}

export default function CatchAVibeClientGate({ onExitToLibrary }: CatchAVibeClientGateProps) {
  const [Game, setGame] = useState<ComponentType<CatchAVibeGameProps> | null>(null);
  ...
  useEffect(() => {
    ...
    const boot = window.setTimeout(() => {
      ...
      failTimer = window.setTimeout(() => {
        if (!cancelled && !loadedRef.current) setLoadErr("Timed out loading Catch A Vibe (45s).");
      }, 45000);

      importCatchAVibeGame()
        .then((m) => {
          if (cancelled) return;
          if (!m?.default) throw new Error("Catch A Vibe module missing default export.");
