...
import type { VibeShiftGameProps } from "./vibe-shift/VibeShiftGame";

const CHUNK_LOAD_ATTEMPTS = 4;
const CHUNK_RETRY_BASE_MS = 400;
const PRODUCT_TITLE_FALLBACK = "Vibe Shift";

async function importVibeShiftGame(): Promise<{ default: ComponentType<VibeShiftGameProps> }> {
  for (let attempt = 0; attempt < CHUNK_LOAD_ATTEMPTS; attempt++) {
    try {
      return await import(
        /* webpackChunkName: "vibe-shift-game", webpackPrefetch: true */ "./vibe-shift/VibeShiftGame"
      );
    } catch (e) {
  ...
  throw new Error("importVibeShiftGame: unreachable");
}
...
function GameLoading({ slow }: { slow: boolean }) {
  return (
    <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-3 px-4 text-center">
      ...
      <p className="max-w-sm font-body text-sm text-white/50">Preparing shift grid and vibe faces.</p>
      {slow ? (
...
class ShiftErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("[Vibe Shift] render error", error, info.componentStack);
  }
    ...
    if (this.state.error) {
      return (
        <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-4 px-4 text-center">
          <p className="font-display text-xl font-black uppercase text-gvc-gold">Grid glitched</p>
...
export interface VibeShiftClientGateProps {
  onExitToLibrary?: () => void;
}

export default function VibeShiftClientGate({ onExitToLibrary }: VibeShiftClientGateProps) {
  const [Game, setGame] = useState<ComponentType<VibeShiftGameProps> | null>(null);
  ...
  useEffect(() => {
    ...
    const boot = window.setTimeout(() => {
      ...
      failTimer = window.setTimeout(() => {
        if (!cancelled && !loadedRef.current) setLoadErr("Timed out loading Vibe Shift (45s).");
      }, 45000);

      importVibeShiftGame()
        .then((m) => {
          if (cancelled) return;
          if (!m?.default) throw new Error("Vibe Shift module missing default export.");
